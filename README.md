...
## আসল, human-sounding ভয়েস চালু করা (Azure Speech — বিনামূল্যে)
ডিভাইসের বিল্ট-ইন Android TTS সবসময় একটু রোবোটিক শোনায়, বিশেষ করে বাংলায়। এর বদলে
আসল, natural মেয়েলি ভয়েস (Microsoft Azure-এর Neural TTS, voice: `bn-BD-NabanitaNeural`)
ব্যবহার করতে চাইলে বিনামূল্যে সেট আপ করা যায়:

1. https://portal.azure.com -এ গিয়ে ফ্রি Microsoft/GitHub অ্যাকাউন্ট দিয়ে লগইন করো
   (নতুন হলে Azure একটা ফ্রি ট্রায়াল অফার করে)।
2. উপরের সার্চ বক্সে **"Speech services"** লিখে সার্চ করো, তারপর **Create** চাপো।
3. Pricing tier-এ **F0 (Free)** বেছে নাও — এতে প্রতি মাসে ৫ লাখ ক্যারেক্টার পর্যন্ত ফ্রি।
4. Region হিসেবে **Central India** বেছে নেওয়াই ভালো (বাংলাদেশ/ভারতের কাছাকাছি, দ্রুত সাড়া দেবে)।
5. তৈরি হয়ে গেলে ওই resource-এর **"Keys and Endpoint"** পেজে গিয়ে **KEY 1** আর
   **Region** কপি করো।
6. অ্যাপে সেটিংসে গিয়ে **Azure Speech Key** আর **Azure Region** ফিল্ডে সেগুলো বসিয়ে
   সেভ করো (region-এ শুধু ছোট হাতের নাম দেবে, যেমন `centralindia`, `eastus` — জায়গার
   পুরো নাম না)।

এটা বসানো থাকলে অ্যাপ প্রথমে এই আসল ভয়েস ব্যবহার করার চেষ্টা করবে, ব্যর্থ হলে (যেমন
ইন্টারনেট না থাকলে) নিজে থেকেই ডিভাইসের ভয়েসে fallback করবে — কিছু ভেঙে যাওয়ার
সুযোগ নেই।
